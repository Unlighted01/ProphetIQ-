...
async def calculate_investment(request: InvestmentRequest):
    price = request.predicted_price_php  # Acts as the total projected construction cost
    down_pct = request.down_payment_pct  # Equity contribution percentage
    rate = request.mortgage_rate         # Annual construction loan / bank financing interest rate
    years = request.mortgage_years       # Amortization term in years

    # 1. Construction Loan / Bank Financing calculation
    ...
    # 2. Rental yield estimate for the completed asset
    # In PH, a conservative estimate for gross rental yield on newly constructed assets is around 6-7% annually.
    # Monthly rent is estimated at roughly 0.6% of the total built asset value.
    ...
    # 4. Built Asset ROI (5-year holding period)
    # Assuming an average 5% annual appreciation of the built asset and land value in Pangasinan
    annual_appreciation = 0.05
    future_value_5yr = price * (1 + annual_appreciation) ** 5
    equity_gained = future_value_5yr - price
    
    initial_investment = price * down_pct  # Firm's initial equity injection
